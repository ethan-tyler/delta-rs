...
fn filter_scan_file_batches_for_selection(
    ...
    for batch in files {
        ...
        for idx in 0..batch.num_rows() {
            let view = LogicalFileView::new(batch.clone(), idx);
            let file_id = normalize_path_as_file_id_with_table_root(
                view.path_raw(),
                &table_root_url,
                "snapshot file path",
            )?;
            mask.push(selection.file_ids.contains(&file_id));
        }

        if mask.iter().any(|&selected| selected) {
            let batch = filter_record_batch(batch, &BooleanArray::from(mask))?;
            if batch.num_rows() > 0 {
                filtered.push(batch);
            }
        }
...
impl TableProvider for DeltaScan {
    ...
    async fn scan(
        ...
        let stream = match &self.snapshot {
            // NOTE: The lazy Snapshot path does not pre-filter file metadata for
            // FileSelection; filtering is deferred to the ScanFileStream replay phase.
            // This means all metadata is scanned even when a small subset is selected.
            ...
            SnapshotWrapper::EagerSnapshot(esn) => {
                if let Ok(files) = esn.files() {
                    let existing_data = if let Some(selection) = self.file_selection.as_ref() {
                        filter_scan_file_batches_for_selection(
                            files,
                            selection,
                    ...
                    scan_plan.scan.scan_metadata_from(
                        ...
                        Box::new(existing_data.into_iter()),
                        None,
                    )
                } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_scan_with_empty_file_selection_scans_no_files() -> TestResult {
        let log_store = TestTables::Simple.table_builder()?.build_storage()?;
        let snapshot = Arc::new(Snapshot::try_new(&log_store, Default::default(), None).await?);
        ...
        visit_execution_plan(baseline_plan.as_ref(), &mut baseline_visitor).unwrap();
        let baseline_num_scanned = baseline_visitor.num_scanned.unwrap_or_default();
        assert!(baseline_num_scanned > 0);

        let selection = FileSelection::new(Vec::<String>::new());
        let provider = DeltaScan::builder()
            .with_snapshot(snapshot)
            .with_file_column(FILE_ID_COLUMN_DEFAULT)
            .build()
        ...
        let mut visitor = DeltaScanVisitor::default();
        visit_execution_plan(plan.as_ref(), &mut visitor).unwrap();
        assert_eq!(visitor.num_scanned, Some(0));
    ...
    async fn test_scan_with_file_selection_strict_missing_files_truncates_output() -> TestResult {
        ...
        for idx in 0..12 {
            selected_file_ids.push(format!(
                "https://urluser:urlpassword@example.com/__does_not_exist_{idx}.parquet?token=urltoken{idx}"
            ));
        }
        ...
        assert!(err_str.contains("File selection contains 12 missing files"));
        assert!(err_str.contains("(and 2 more)"));
        assert!(!err_str.contains("urluser"));
        assert!(!err_str.contains("urlpassword"));
        assert!(!err_str.contains("urltoken"));
    ...
    #[tokio::test]
    async fn test_scan_with_file_selection_uses_eager_snapshot_path() -> TestResult {
        let log_store = TestTables::Simple.table_builder()?.build_storage()?;
        let snapshot = Arc::new(Snapshot::try_new(&log_store, Default::default(), None).await?);
        let table_root = snapshot.scan_builder().build()?.table_root().clone();
        let eager_snapshot =
            Arc::new(EagerSnapshot::try_new_with_snapshot(log_store.as_ref(), snapshot).await?);

        let session = Arc::new(create_session().into_inner());
        let state = session.state_ref().read().clone();

        let selected_file_ids: Vec<String> = eager_snapshot
            .file_views(log_store.as_ref(), None)
            .take(1)
            .map_ok(|view| table_root.join(view.path_raw()).unwrap().to_string())
            .try_collect()
    ...
    #[test]
    fn test_file_selection_from_adds_matches_from_paths() -> TestResult {
        let table_root = Url::parse("file:///tmp/delta/").unwrap();
        ...
        let from_adds = FileSelection::from_adds([add], &table_root)?;
